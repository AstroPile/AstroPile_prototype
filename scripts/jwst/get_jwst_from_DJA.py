...
def _cut_stamps_fn(directory_path,output_directory,phot_table,field_identifier,subsample='all'):
    ...
    if subsample == 'all':
        ...
        subsample_indices = range(len(phot_table))  # This will create an iterable over all indices
    else:
    ...
    for f in _filter_list:
        ...
        for file_path in matching_files:
            ...
            #print(f)
            pickle_filename = output_directory+'jwst_'+field_identifier+'_'+f+'_sample_'+str(len(subsample_indices))+'_forastropile.pkl'  # Update the path as needed
...
keycols = []
for i in phot_table.keys():
    try:
        print(i, len(phot_table[i][0]))
    except:
        keycols.append(i)
  
_FLOAT_FEATURES = keycols
